...
def extract_attention_weights(bert_model, input_ids, attention_mask, tokenizer):
    ...
    avg_attention = last_layer.mean(dim=0)  # average over heads: shape (seq_len, seq_len)
    cls_attention = avg_attention[0]  # attention from CLS token to all other tokens
