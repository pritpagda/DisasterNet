...
def predict(text: str, image_bytes: bytes, image_filename: str, user_id: int = None, db: Session = None):
    ...
    if db is not None and user_id is not None:
        prediction = Prediction(user_id=user_id, text=text, informative=prediction_result.get("informative"),
                                humanitarian=prediction_result.get("humanitarian"),
                                damage=prediction_result.get("damage"), image_path=image_filename, error=None, )
        db.add(prediction)
        db.commit()
