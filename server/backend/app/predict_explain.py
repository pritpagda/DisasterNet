...
from .pred import (informative_model, humanitarian_model, damage_model, tokenizer, image_transform, )
...
def predict_explain(text: str, image_bytes: bytes, user_id: int = None, db=None, image_filename: str = None):
    prediction_result = predict(text=text, image_bytes=image_bytes, user_id=user_id, db=db,
    ...
    text_explanations["informative"] = extract_attention_weights(informative_model.bert, input_ids, attention_mask,
                                                                 tokenizer)
    ...
    if prediction_result.get("humanitarian"):
        ...
        text_explanations["humanitarian"] = extract_attention_weights(humanitarian_model.bert, input_ids,
    ...
    return {**prediction_result, "image_explanations": image_explanations, "text_explanations": text_explanations, }
